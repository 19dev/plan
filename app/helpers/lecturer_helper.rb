...
module LecturerHelper
  ...
  def lectureradd
    ...
    session[:success] = "#{lecturer.full_name} isimli kişi öğretim üyesini olarak eklendi"
  ...
  def lecturerdel
    ...
    if assignments != []
      session[:error] = "Bu öğretim üyesinin ders ataması vardır, bu yüzden silemezsiniz. " +
    ...
    session[:success] = "Öğretim üyesini başarıyla silindi"
  ...
  def lecturerupdate
    ...
    session[:success] = "#{Lecturer.find(session[:lecturer_id]).full_name} isimli öğretim üyesini başarıyla güncellendi"
    redirect_to '/user/lecturershow'
  end
end
