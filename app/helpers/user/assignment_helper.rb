...
module User
  module AssignmentHelper
    ...
    def assignmentadd
      if flash[:error] = control({ params[:course_ids] => "Atanacak ders", params[:lecturer_id] => "Dersi atanacak hoca",})
      ...
      params[:course_ids].each do |course_id|
        ...
        # v1.1
        # unless Assignment.find_by_lecturer_id_and_course_id_and_period_id(params[:lecturer_id], course_id, session[:period_id])
      ...
      flash[:success] = "#{Lecturer.find(params[:lecturer_id]).full_name} öğretim üyesinin dersleri atandı"
    ...
    def assignmentdel
      ...
      flash[:success] = "#{Lecturer.find(session[:lecturer_id]).full_name} öğretim üyesinin dersleri silindi"
    ...
    def assignmentupdate
      if params[:course_ids]
        ...
        flash[:success] = "#{Lecturer.find(session[:lecturer_id]).full_name} öğretim üyesinin dersleri güncellendi"

    else
        flash[:error] = "#{Lecturer.find(session[:lecturer_id]).full_name} isimli öğretim üyesinin dönemlik tüm derslerini ve " +
